...
func sessionKeyFromFilename(name string) (string, bool) {
	...
	case strings.HasSuffix(name, ".meta.json"):
		return sessionKeyFromStem(strings.TrimSuffix(name, ".meta.json")), true
	case strings.HasSuffix(name, ".index.json"):
		return sessionKeyFromStem(strings.TrimSuffix(name, ".index.json")), true
	case strings.HasSuffix(name, ".active.jsonl"):
		return sessionKeyFromStem(strings.TrimSuffix(name, ".active.jsonl")), true
	case strings.HasSuffix(name, ".jsonl") && !strings.Contains(name, ".deleted."):
		if m := archiveSegmentRe.FindStringSubmatch(name); len(m) == 3 {
			return sessionKeyFromStem(m[1]), true
		}
		return sessionKeyFromStem(strings.TrimSuffix(name, ".jsonl")), true
	default:
		return "", false
	}
}
...
func parseArchiveSegmentFilename(name, key string) (int, bool) {
	...
	if len(m) != 3 || m[1] != sessionFileStem(key) {
		return 0, false
	}
...
func activeSegmentFilename(key string) string { return sessionFileStem(key) + ".active.jsonl" }
func legacySegmentFilename(key string) string { return sessionFileStem(key) + ".jsonl" }
func archivedSegmentFilename(key string, seq int) string {
	return fmt.Sprintf("%s.%04d.jsonl", sessionFileStem(key), seq)
}

func (sm *SessionManager) sessionMetaPath(key string) string {
	return filepath.Join(sm.storage, sessionFileStem(key)+".meta.json")
}
func (sm *SessionManager) sessionIndexPath(key string) string {
	return filepath.Join(sm.storage, sessionFileStem(key)+".index.json")
}

func sessionFileStem(key string) string {
...
func sessionKeyFromStem(stem string) string {
	replacer := strings.NewReplacer(
		"%2F", "/",
		"%5C", "\\",
		"%3A", ":",
